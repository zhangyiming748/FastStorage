...
func SetMysql(user, password, host, port, dbName string) {
	...
	dsnWithoutDB := fmt.Sprintf("%s:%s@tcp(%s:%v)/?charset=utf8&loc=Asia%%2FShanghai", user, password, host, port)
	...
	// 第二次连接：指定数据库进行正式连接
	dsnWithDB := fmt.Sprintf("%s:%s@tcp(%s:%v)/%s?charset=utf8&loc=Asia%%2FShanghai", user, password, host, port, dbName)
